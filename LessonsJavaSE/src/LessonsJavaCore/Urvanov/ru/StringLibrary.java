 ...
 *
 * StringBuilder и StringBuffer. Оба обладают достаточно богатым набором функций для модификации строк - в этом       *  StringBuffer
 * * * случае объекты действительно модифицируются. В чем же разница? Разница в том, что класс StringBuilder
 * * * не является потокобезопасным (threadsafe). Мы пока не касались вопросов многопоточности (это очень сложная
 * * * и интересная тема), но в двух словах можно сказать так — при попытке одновременного изменения объекта из
 * * * нескольких потоков объект StringBuilder не гарантирует корректного результата. Если на бытовом уровне - если
 * * * два человека одновременно пытаются собирать один кубик Рубика, то ничего хорошего не получится. Ели же они
 * * * это делают последовательно, соблюдая очередность, то результат может быть получен, правда не так быстро. Так вот
 * * * класс StringBuilder — это класс для одного потока и если вам требуется по каким-то причинам модифицировать его
 * * * из нескольких потоков, то вам надо использовать класс StringBuffer.
 * * * Вопрос - какой класс из этих двух использовать - решается достаточно просто. Если поток один, то StringBuilder.
 * * * За счет игнорирования потокобезопасности он работает очень быстро. Но если вам нужно модифицировать объект из
 * * * разных потоков, то используйте StringBuffer.
 *
 * В отличие от String у StringBuilder кроме длины есть ещё вместимость/ёмкость (capacity). Вместительность можно        StringBuilder
