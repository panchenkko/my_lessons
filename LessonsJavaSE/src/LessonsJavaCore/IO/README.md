...
Эта реализация потока вывода имеет назначенного размера память (буфер), эту память можно самому установить, а можно
доверить реализации по умолчанию.

При вызове метода write(), всё записывается в буфер этого класса, когда этот буфер будет заполнен или же для записи 
не будет достаточно места он обратится к следующей реализации OutputStream (какая изначально была назначена как 
основная), например FileOutputStream, он вызовет у этого класса метод write() и запишет все накопившиеся данные в файл 
и освободит буфер в классе BufferedOutputStream, после чего в свободный буфер запишет данные, для каких не хватало 
места. 

При вызове метода flush(), неважно сколько занято или сколько свободно места в буфере, все данные оттуда передадутся 
основной реализации OutputStream, какая запишет все данные в назначенное место.